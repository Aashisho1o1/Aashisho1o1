# Hi there, I'm Aashish Sunar 👋

## Full Stack Developer

MS in Information Systems & Technology @ Claremont Graduate University (4.0 GPA) | Full Stack Developer @ Soka University

Currently building AI-powered applications with production-grade ML pipelines and scalable backend systems. Passionate about leveraging LLMs, natural language processing, and intelligent automation to solve real-world problems.

---

### 💼 What I'm Working On

- 🎯 **OwenWrites** - AI writing platform helping fiction writers maintain character consistency ([www.owenwrites.co](https://www.owenwrites.co))
- 🏥 Building an academic advising chatbot at CISAT, Claremont Graduate University
- 🌍 Operating a family e-commerce jewelry business with modern tech stack

---

### 🛠️ Tech Stack

**Languages & Frameworks**
```
Frontend:  React • TypeScript • JavaScript • HTML/CSS • Tailwind
Backend:   Python • Node.js • Flask • Express.js
AI/ML:     LLMs • OpenAI API • NLP • TensorFlow • ML Pipelines
Database:  SQL • PostgreSQL • SQLite • MongoDB
Tools:     Git • AWS • REST APIs • Docker
```

**Currently Learning:** Advanced System Design • MLOps • Distributed Systems • Java

---

### 🌟 Featured Projects

#### [🎭 OwenWrites - AI Writing Platform](https://github.com/Aashisho1o1/dialogue-consistency-checker)
Production-ready ML pipeline for character voice consistency detection in fiction writing
- Built an advanced dialogue analysis system using LLMs and NLP
- Created persona-based chatbot featuring classic authors (Maya Angelou, Hemingway, etc.)
- Selected for UCR INNOVAR Entrepreneurship Program
- **Tech:** TypeScript, Chrome Built-in Prompt API, ML, React

#### [🏥 Hospital Search & Medical Document Analysis](#)
Full-stack healthcare search application with intelligent filtering
- Python Flask backend + React frontend serving real hospital data
- SQLite database with fuzzy search supporting partial matches
- Cross-functional team collaboration (5 developers)
- **Tech:** Python, Flask, React, SQLite, REST APIs

#### [🌍 Alumni Disaster Tracker](https://github.com/Aashisho1o1/alumni-tracker)
Real-time emergency alert system serving 2,000+ users globally
- Responsive UI with live disaster notifications for university alumni
- Reduced emergency response coordination time by enabling rapid contact
- **Tech:** React, JavaScript, RESTful APIs

#### [💎 Jewelry E-commerce Platform](#)
Full-stack e-commerce solution with payment integration
- Secure payment gateway integration and inventory management
- Optimized relational database schema for product catalog
- Successfully operating a business generating consistent revenue
- **Tech:** JavaScript, HTML/CSS, SQL, Payment APIs

---

### 📊 GitHub Stats

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Aashisho1o1&layout=compact&theme=radical&hide_border=true)

---

### 🎓 Background

- **MS in Information Systems and Technology** | AI-Driven Innovation | GPA: 4.0
  - Claremont Graduate University (Expected May 2026)
  - Coursework: Software Development, Database Systems, Natural Language Processing
  
- **BA in Economics and Finance** | Soka University of America
  - Former Student Government President
  - 2.5+ years full-stack development experience

---

### 🏆 Achievements

- 🎯 Selected for **UCR INNOVAR Entrepreneurship Program** (OwenWrites)
- 🌍 Built disaster tracking system serving **2,000+ global users**
- 📈 Reduced inventory search time by **50%** through algorithmic optimization
- 💼 Co-founded revenue-generating e-commerce business
- 🎖️ Maintained **4.0 GPA** while working full-time and building startups

---

### 🌍 Languages

English • Hindi • Nepali • Mandarin Chinese

---

### 📫 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)]([https://linkedin.com/in/your-profile](https://www.linkedin.com/in/aashish-sunar/))
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:aashish.sunar@cgu.edu)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=About.me&logoColor=white)](https://www.owenwrites.co)

---

💡 **Open to full-time Software Engineering opportunities** | OPT/CPT visa sponsorship needed

⚡ **Fun fact:** I'm an aspiring novelist working on an epic fantasy novel while building AI tools for writers!

---

<div align="center">
  <i>Thanks for visiting my profile! Feel free to explore my repositories and reach out if you'd like to collaborate on exciting projects.</i>
</div>
